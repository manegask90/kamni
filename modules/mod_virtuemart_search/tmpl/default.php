...
<!--BEGIN Search Box -->
<form action="<?php echo JRoute::_('index.php?option=com_virtuemart&view=category&search=true&limitstart=0&virtuemart_category_id='.$category_id ); ?>" method="get" class="search-products-lite">
 ...
 $image = JURI::base() . $imagepath;
			...
			if ($button) :
			    if ($imagebutton && $imagepath) :
			        $button = '<button style="vertical-align:middle" type="image" value="'.$button_text.'" class="button'.$moduleclass_sfx.'" src="'.$image.'" onclick="this.form.keyword.focus();" ></button>';
			    else :
